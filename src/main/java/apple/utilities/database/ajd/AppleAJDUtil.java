...
public interface AppleAJDUtil extends FileIOJoined {
    default <DBType extends SaveFileable, TaskExtra> Placeholder
    saveInFolderNow(File folder, DBType saveThis, AsyncTaskQueueStart<TaskExtra> queue) {
        return saveInFolder(folder, saveThis, queue).complete();
    }

    default <DBType extends SaveFileable, TaskExtra> AsyncTaskAttempt<Placeholder, TaskExtra>
    saveInFolder(File folder, DBType saveThis, AsyncTaskQueueStart<TaskExtra> queue) {
        return this.save(saveThis.fileWithParent(folder), saveThis, queue);
    }

    default <TaskExtra, DBType> void
    saveNow(File file, DBType saveThis, AsyncTaskQueueStart<TaskExtra> queue) {
        this.save(file, saveThis, queue).complete();
    }

    default <TaskExtra, DBType> AsyncTaskAttempt<Placeholder, TaskExtra>
    save(File file, DBType saveThis, AsyncTaskQueueStart<TaskExtra> queue) {
        SupplierUncaught<Placeholder> serializer = serializer(file, saveThis);
        file.getParentFile().mkdirs();
        return queue.accept(serializer);
    }

    default <DBType, TaskExtra> Collection<DBType>
    loadFolderNow(File folder, Class<DBType> dbType, AsyncTaskQueueStart<TaskExtra> queue) {
        return loadFolder(folder, dbType, queue).stream().map(AsyncTaskAttempt::complete).toList();
    }

    default <DBType, TaskExtra> Collection<AsyncTaskAttempt<DBType, TaskExtra>>
    loadFolder(File folder, Class<DBType> dbType, AsyncTaskQueueStart<TaskExtra> queue) {
    ...
    default <DBType, Extra> DBType
    loadNow(File file, Class<DBType> type, AsyncTaskQueueStart<Extra> queue) {
        return this.load(file, type, queue).complete();
    }

    default <DBType, Extra> AsyncTaskAttempt<DBType, Extra>
    load(File file, Class<DBType> type, AsyncTaskQueueStart<Extra> queue) {
