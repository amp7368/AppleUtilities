...
public interface AsyncTaskCreate<Extra> {
    ...
    default AsyncTaskQueueStart<Extra> withHandler(TaskHandler<Extra> handler) {
        ...
        return new AsyncTaskQueueStart<Extra>() {
            @Override
            public <T> AsyncTaskAttempt<T, Extra> accept(SupplierUncaught<T> task, Consumer<T> onSuccess, Consumer<Exception> onFailure) {
