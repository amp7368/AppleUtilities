...
public interface AsyncTaskQueueStart<Extra> {
    ...
    <T> AsyncTaskAttempt<T, Extra> accept(SupplierUncaught<T> task, Consumer<T> onSuccess,
    ...
    default <T> AsyncTaskAttempt<T, Extra> accept(SupplierUncaught<T> task) {
        return this.accept(task, null, Exception::printStackTrace);
    }

    default <T> AsyncTaskAttempt<T, Extra> acceptCaught(Supplier<T> task, Consumer<T> onSuccess,
        Consumer<Exception> onFailure) {
